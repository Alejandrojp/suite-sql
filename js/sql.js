...
export function generarDelExcelPlantillas() {
    try {
        ...
        if (modo === 'manual') {
            ...
            const caseEstadoNoProv = `CASE \n        WHEN PlantillasFiltradas.codigo IS NULL THEN 'YA NO ESTÁ EN NINGUNA (OK)'\n        ELSE 'SE BORRARÁ' \n    END AS Estado`;

            auditSql = `SELECT \n    ListaTiendas.t_num AS 'Nº Tienda', \n    COALESCE(PlantillasFiltradas.codigo, '---') AS Codigo_Plantilla, \n    COALESCE(PlantillasFiltradas.nombre, 'NINGUNA PLANTILLA') AS Nombre_Plantilla, \n    ListaArticulos.idArticulo AS Articulo, \n    COALESCE(MA.descripcion_principal, 'NO COPIADO EN EMPRESA') AS Descripcion, \n    ${caseEstadoNoProv}\nFROM (\n    ${unionTiendas}\n) ListaTiendas\nCROSS JOIN (\n    ${unionArticulos}\n) ListaArticulos\nLEFT JOIN (\n    SELECT d2.articulo, c2.codigo, c2.nombre\n    FROM detplantilla d2\n    INNER JOIN cabplantilla c2 ON c2.codigo = d2.codigo\n    WHERE c2.situacion = 'A'\n) AS PlantillasFiltradas ON PlantillasFiltradas.articulo = ListaArticulos.idArticulo AND PlantillasFiltradas.nombre LIKE ListaTiendas.c_nom\nLEFT JOIN maeart MA ON MA.codigo = ListaArticulos.idArticulo AND MA.empresa = ListaTiendas.e_cod\nORDER BY Estado ASC, ListaTiendas.t_num, PlantillasFiltradas.nombre, ListaArticulos.idArticulo;`;
        } else if (modo === 'excel_tienda') {
