...
function inyectarSQLPlantillas(containerId, codeId, sql) {
    ...
    // GUARDAMOS EN MEMORIA GLOBAL (Para evitar límites del DOM en scripts gigantes)
    window.fullSqlCache = window.fullSqlCache || {};
    window.fullSqlCache[codeId] = sql;
    
    // MOSTRAR SIN LÍMITES EN PANTALLA (Para copiar a mano)
    codeEl.textContent = sql;
    
    codeEl.dataset.fullSql = "MEM_CACHE"; // Marcador
...
export function generarSQLMasivo() {
    ...
    if (isExcel) {
        ...
        datosExcel = validData.map(d => ({ idArticulo: d.art, grupo: d.grp }));
        listaArticulos = [...new Set(validData.map(d => d.art))]; 
    } else {
...
export function generarSQLBorrar() {
    ...
    if (isExcel) {
        ...
        listaArticulos = textArticulos.split(/[\s,]+/).map(s => s.replace(/\D/g,'')).filter(s => s !== '');
...
export function generarSQLSwap() {
    ...
    if (isExcel) {
        ...
        validData.forEach(d => {
            pairs.push({ old: sqlEscape(d.oldId), new: sqlEscape(d.newId) });
            if (!allNewIds.includes(d.newId)) allNewIds.push(d.newId);
        });
    } else {
        ...
        rows.forEach(row => {
            ...
            if (oldId && newId) {
                pairs.push({ old: oldId.replace(/'/g, "''"), new: newId.replace(/'/g, "''") });
                if (!allNewIds.includes(newId)) allNewIds.push(newId);
            }
        });
    }
...
export function generarAddPlantillas() {
    ...
    if (modo === 'manual') {
        ...
        let stock = document.getElementById('prov-stock').value || 0;
        let listaTiendas = Array.from(document.querySelectorAll('#list-add .store-item input:checked')).map(cb => cb.value);
        ...
        data.forEach(d => {
            ...
            let art = sqlEscape(d.art);
            let stock = sqlEscape(d.stock);
        ...
        let uniqueArts = [...new Set(data.map(d => d.art))];
        let unionArticulosOthers = uniqueArts.map((art, idx) => idx === 0 ? `SELECT '${sqlEscape(art)}' AS idArt` : ` UNION ALL SELECT '${sqlEscape(art)}'`).join('');
        ...
        for (let i = 0; i < data.length; i += batchSize) {
            ...
            let unionTemp = chunk.map((d, idx) => {
                return idx === 0 ? `SELECT '${sqlEscape(d.cod)}' AS cod, '${sqlEscape(d.art)}' AS art, '${sqlEscape(d.stock)}' AS stock` : ` UNION ALL SELECT '${sqlEscape(d.cod)}', '${sqlEscape(d.art)}', '${sqlEscape(d.stock)}'`;
            }).join('');
        ...
        let unionIdealCod = data.map((d, idx) => {
            if (idx === 0) return `SELECT '${sqlEscape(d.cod)}' AS cod, '${sqlEscape(d.art)}' AS idArticulo, '${sqlEscape(d.stock)}' AS stock`;
            return ` UNION ALL SELECT '${sqlEscape(d.cod)}', '${sqlEscape(d.art)}', '${sqlEscape(d.stock)}'`;
        }).join('\n    ');
        ...
        let uniqueCods = [...new Set(data.map(d => d.cod))];
...
export function generarDelExcelPlantillas() {
    ...
    if (modo === 'manual') {
        ...
        let safeProv = sqlEscape(proveedor).replace(/\s+/g, '%');
        let listaTiendas = Array.from(document.querySelectorAll('#list-del .store-item input:checked')).map(cb => cb.value);
        ...
        let orConds = listaTiendas.map(dbId => {
            const sObj = state.tiendasData.find(t => t.id === dbId);
            let tNum = sObj.name.split(' - ')[0].trim();
            // Ya NO usamos e.codigo, solo el nombre directo de la plantilla
            return `c.nombre LIKE '%(T${tNum})%${safeProv}%'`;
        }).join('\n        OR ');

        let artsJoined = listaArticulos.map(a => `'${sqlEscape(a)}'`).join(', ');
        ...
        data.forEach(d => {
            ...
            // Query Delete simplificada sin maecli
        ...
        let batchSize = 1000; 
        let values = data.map(d => `(d.codigo = ${d.cod} AND d.articulo = ${d.art})`);
        ...
        let unionIdealCod = data.map((d, idx) => {
            if (idx === 0) return `SELECT '${sqlEscape(d.cod)}' AS cod, '${sqlEscape(d.art)}' AS idArticulo`;
            return ` UNION ALL SELECT '${sqlEscape(d.cod)}', '${sqlEscape(d.art)}'`;
        }).join('\n    ');
...
export function generarVaciarPlantillas(codigosRaw) {
    let codigos = [...new Set(codigosRaw)];
...
export function generarUpdateNombrePlantillas() {
    ...
    let sql = data.map(d => `UPDATE cabplantilla SET nombre = '${sqlEscape(d.nombre)}' WHERE codigo = ${d.cod};`).join('\n');
...
export function generarConsultaPlantillas() {
    let prov = document.getElementById('cons-prov').value.trim();
    let clientes = document.getElementById('cons-clientes').value.split(/[\r\n,]+/).map(s => s.trim()).filter(s => s !== '' && !isNaN(s));
    if (!prov || clientes.length === 0) return showNotification("⚠️ Debes indicar el ID del Proveedor y Clientes.");

    let sql = `SELECT \n    c.codigo, \n    c.nombre, \n    d.articulo, \n    d.stock_ideal,\n    c.cliente\nFROM \n    cabplantilla c\nINNER JOIN \n    detplantilla d ON c.codigo = d.codigo\nWHERE \n    c.situacion = 'A'\n    AND c.proveedor = ${sqlEscape(prov)}\n    AND c.cliente IN (\n        ${clientes.join(', ')}\n    );`;
    inyectarSQLPlantillas('res-consulta', 'out-consulta', sql);
    
    // Aquí usamos el marcador MEM_CACHE para la consulta SELECT también.
    ...
    outputSections.forEach(outputSection => {
        if (outputSection.style.display !== 'none' && outputSection.style.display !== '') {
            ...
            codeBlocks.forEach((block, index) => {
                ...
                // LEEMOS DE LA MEMORIA PURA SIEMPRE
